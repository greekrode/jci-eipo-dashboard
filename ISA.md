...
- 2026-06-23 (refined): Explorer gained a Members column (member codes joined, hover tooltip lists each member's full name via `brokerNames()`); legend now covers leads + members. Whole UI scaled up (base 15px, table 13.5px, stat values 26px, card titles 15px, tabs 13px, larger chart ticks/legends/heights) and the layout switched to full-viewport width (max-w 1240 → 2100, px-6) so it fills the screen instead of a centered column.
